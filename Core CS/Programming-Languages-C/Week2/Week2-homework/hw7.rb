...
class Point < GeometryValue
  ...
  def inbetween(v,end1,end2)
    ((end1 - Epsilon <= v) and (end2 + Epsilon >= v)) or ((end2 - Epsilon <= v) and (end1 + Epsilon >= v))
  end
...
class Line < GeometryValue
  ...
  def intersectLine line
    if real_close(m, line.m)
      ...
      int_x = (line.b - b) / (m - line.m)
      int_y = m*int_x + b
      Point.new(int_x, int_y)
    end
  end
...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    equal_x = real_close(x1, x2)
    equal_y = real_close(y1, y2)
    if equal_x and equal_y
      Point.new(x1, y1)
    else
      if equal_x or x1 < x2
        if y1 > y2
      ...
      else LineSegment.new(x2,y2,x1,y1)
      end
    end
  end
  ...
  def intersectWithSegmentAsLineResult seg
    ...
    if real_close(x1start,x1end)
      # Both Line segments are on a vertical line
      ...
      if real_close(aYend,bYstart)
        Point.new(aXend,aYend) # Segments just touching
      elsif aYend < bYstart
        NoPoints.new # disjoint
      elsif aYend > bYend
        LineSegment.new(bXstart,bYstart,bXend,bYend) # One inside another
      else
        LineSegment.new(bXstart,bYstart,aXend,aYend) # Overlapping
      end
    else
      # Both line segments are on a non-vertical line
      ((aXstart,aYstart,aXend,aYend),
